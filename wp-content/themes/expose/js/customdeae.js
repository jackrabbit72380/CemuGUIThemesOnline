...
// -------------------------------------------------------------------------------------------
	...
	jQuery('.submit_form').kriesi_ajax_form({sendPath:url});	// activates submit content form
...
(function($)
{
	$.fn.galleryDisplay = function(options) 
	{
		...
		return this.each(function()
		{
			...
			itemContainer.attr('id','item_medium_gallery');
...
(function($)
{
	$.fn.kriesi_image_preloader = function(variables, callback) 
	{
		...
		{
			var container 	= $(this),
				images		= $('img', this).not('.no_preload').css({opacity:0, visibility:'visible', display:'block'}),
...
function k_menu()
{
	...
	jQuery("#nav a, .nav a").removeAttr('title');
	jQuery(" #nav ul, .nav ul ").css({display: "none"}); // Opera Fix
	jQuery("#nav li:first, .nav li:first").attr('id','home');
	
	//smooth drop downs
	jQuery("#nav li, .nav li").each(function()
	{	
